core.matrix.model <- function(Time, State, Parameters) {
  with(as.list(c(State, Parameters)), {
    ...
    dS.c<-N.c*(rmax.c+d.c)*(1-N.c/(k.c*(1.01-phi))) - ((S.c*beta.c*I.c)/N.c^kappa+(epsilon*beta.mp*beta.cp*I.m*S.c)/(N.c+epsilon*N.m)^kappa) - d.c*S.c 
    dI.c<-(beta.cp*S.c*I.c)/N.c^kappa+(epsilon*beta.mp*beta.cp*S.c*I.m)/(N.c+epsilon*N.m)^kappa - I.c*(alpha.c*d.c+d.c+gamma.c)
    dR.c<-I.c*gamma.c-R.c*d.c
    
    dS.m<-N.m*(rmax.m+d.m)*(1-N.m/(k.m*phi)) - ((beta.mm*S.m*I.m)/N.m^kappa+(epsilon*beta.mm*beta.cm*S.m*I.c)/(epsilon*N.c+N.m)^kappa) - d.m*S.m 
    dI.m<-((beta.mm*S.m*I.m)/N.m^kappa+(epsilon*beta.mm*beta.cm*S.m*I.c)/(epsilon*N.c+N.m)^kappa) - I.m*(alpha.m*d.c+d.m+gamma.m)
