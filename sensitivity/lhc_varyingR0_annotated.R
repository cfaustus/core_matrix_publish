...
s1$kappa=0 # density dependent
# R0_C = 1.5 # R0 in core at phi = 0
# R0_M = 0.5 # R0 in matrix at phi = 1.0

## calculate beta.cp and beta.mm based on 100% forest cover
s1$beta.c <- s1$R0_C*(s1[['gamma.c']]+s1[['alpha.c']]*s1[['d.c']]+s1[['d.c']])/(s1[['k.c']])
s1$beta.m <- s1$R0_M*(s1[['gamma.m']]+s1[['alpha.m']]*s1[['d.m']]+s1[['d.m']])/(s1[['k.m']]) # is this right?
...
write.csv(s1,"sensitivity/lhc/lhc_humans_dd_R0vary_phi0.9.csv", row.names = F)
