...
# raina & christina
########
s1<- read.csv("sensitivity/lhc/lhc_humans_fd_R0vary_phi0.5.csv",header=T) # dataframe of random parameters
  ...
  core.matrix.model <- function(Time, State, Parameters) {
    with(as.list(c(State, Parameters)), {
      ...
      dS.c <- ifelse((1 - N.c/(k.c*(1.01-phi))) > 0, #IF UNDER CARRYING CAPACITY THEN..
                     ((N.c)*(b.c)*(1 - N.c/(k.c*(1.01-phi))) - ((beta.c*S.c*I.c)/N.c^kappa
                     ...
                     (- ((beta.c*S.c*I.c)/N.c^kappa + (epsilon*psi*beta.m*S.c*I.m)/(N.c+epsilon*N.m)^kappa) 
                      - d.c*S.c + sigma.c*R.c)) 
      dI.c <- (beta.c*S.c*I.c)/N.c^kappa + (epsilon*phi*beta.m*S.c*I.m)/(N.c+epsilon*N.m)^kappa  - I.c*(alpha2.c+d.c+gamma.c)
      dR.c <- I.c*gamma.c - R.c*(d.c+sigma.c)
      
      dS.m <-  ifelse((1 - N.m/(k.m*phi)) > 0,
                      ((N.m)*(b.m)*(1 - N.m/(k.m*phi)) - ((beta.m*S.m*I.m)/N.m^kappa
      ...
      dI.m <- ((beta.m*S.m*I.m)/N.m^kappa + (epsilon*psi*beta.c*S.m*I.c)/(epsilon*N.c+N.m)^kappa) - I.m*(alpha2.m+d.m+gamma.m)
